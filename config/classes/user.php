...
class User
{
    ...
    public static function setBodyColors($user_id, $colors)
    {
        global $pdo;
        $stmt = $pdo->prepare("UPDATE users SET bodycolors = :colors WHERE id = :user_id");
        $stmt->bindValue(":user_id", $user_id, PDO::PARAM_INT);
        $stmt->bindValue(":colors", $colors, PDO::PARAM_STR);
        $stmt->execute();
    }
}
