...
include($_SERVER['DOCUMENT_ROOT'] . '/Login/UserInfo.php');
$baseUrl = "https://www.rovive.pro";
...
switch(true){
	case($name !== null):
		$UserPlaces = $pdo->prepare("SELECT * FROM games WHERE creator_id = :cid");
		$UserPlaces->execute([':cid' => $id]);
		$UserRows = $UserPlaces->fetchAll();
		break;
}

$AllFetch = $pdo->prepare("SELECT * FROM templates WHERE approved = '1' AND public = '1' AND rsprice = '0' AND tkprice = '0' AND genre = 'templateBasic' AND itemtype = 'place'");
$AllFetch->execute();
$StrategyFetch = $pdo->prepare("SELECT * FROM templates WHERE approved = '1' AND public = '1' AND rsprice = '0' AND tkprice = '0' AND genre = 'templatestrategy' AND itemtype = 'place'");
$StrategyFetch->execute();
$ActionFetch = $pdo->prepare("SELECT * FROM templates WHERE approved = '1' AND public = '1' AND rsprice = '0' AND tkprice = '0' AND genre = 'templateaction' AND itemtype = 'place'");
$ActionFetch->execute();

$AllRows = $AllFetch->fetchAll();
						 ...
						 <div id='header-login-wrapper' class='iframe-login-signup' data-display-opened=''>
							...
							<span id='header-or'>or</span>
								<span class='studioiFrameLogin'>
									...
									<iframe class='login-frame' src='". $baseUrl ."/Login/iFrameLogin.aspx' scrolling='yes' frameborder='0'></iframe>
								</span>
							</div>
						 </div>
			";
            ...
            <h2 id="StudioGameTemplates">GAME TEMPLATES</h2>
               <div class="templatetypes">
                  <ul class="templatetypes">
                     <li js-data-templatetype="All"><a href="#All">All</a></li>
                     <li js-data-templatetype="Strategy"><a href="#Strategy">Theme</a></li>
                     <li js-data-templatetype="Action"><a href="#Action">Gameplay</a></li>
                  </ul>
               </div>
               <div class="templates" js-data-templatetype="All">
			<?php
			switch(true){
				case ($AllRows):
					foreach($AllRows as $GameInfo){
						echo "
					...
					switch (true){
						case ($UserRows):
							foreach($UserRows as $GameInfo){
								echo "
								  <div class='template' placeid='". $GameInfo['id'] ."'>
									 <a href='' class='game-image'><img width='197' height='115' class='' src='". $baseUrl ."/Tools/Asset.ashx?id=". $GameInfo['id'] ."&request=game' /></a>
									 <p>". $GameInfo['title'] ."</p>
								  </div>
								";
							}
